...
uint shadow_texture_width = 2048;
float shadow_pixel_size = 1.0f / 48.0f;
float shadow_growth = 8.0f;
...
void shadow_render(std::vector<shadow_map>& shadow_maps, uint camera) {
    ...
    auto vfunc = [](int t) {
        return shadow_texture_width * shadow_pixel_size * pow(shadow_growth, t);
    };
    ...
    for(auto& map : shadow_maps) map.framebuffer.resize(ivec2(shadow_texture_width));
    ...
    for(int i = 0; i < shadow_maps.size(); ++i) {
        float width = shadow_pixel_size * shadow_texture_width * pow(8.0f, i);
        float texel_size = width / shadow_texture_width;
...
void base_render(uint camera, axiom::render_target& f) {
    ...
    glUniform1f(19, shadow_texture_width);
    glUniform1f(20, shadow_pixel_size);
    glUniform1f(21, shadow_growth);
...
void create_ui() {
    ...
    std::function<void(axiom::tab_widget*)> func_settings = [ui_system, node, msystem](axiom::tab_widget* self) {
        ...
        axiom::slider_widget::insert(vec2(120, 16), 6, axiom::color_blue, vec2(0.0f, 1.0f), 0.01f, 0.98f, "",
            [msystem](axiom::slider_widget& self) {
                if(!self.pressed) self.current_value = msystem->light_contrast;
                else msystem->light_contrast = self.current_value;

                self.text[0]->string = axiom::to_base(self.current_value, 10, 2);
            }
        );
        ui_system->set_attrib(vec2(0, 16), vec2(FLT_MAX, 16), vec2(1.0f));
        ui_system->input_step();
    ...
    axiom::button_widget::insert(vec2(32.0f), axiom::color_red, vec4(96, 68, 12, 12), 
        [ui_system](axiom::button_widget& self) {
            if(self.pressed) {
                std::vector<uint> erase_entities;
                for(uint entity : axiom::get_system<axiom::physics_system3d>().collectors[0].entities) {
                    axiom::collider3d& collider = axiom::get_component<axiom::collider3d>(entity);
                    if(!collider.is_static) erase_entities.push_back(entity);
                }

                for(uint entity : erase_entities) axiom::global_core.ecs->erase_entity(entity);
                ...
                float w = 1.0f;
                vec3 start_pos = vec3(0xC, 0x15B, 0xF8);
                        ...
                        for(int y = 0; y < array.y; ++y) {
                            for(int z = 0; z < array.z; ++z) {
                                ...
                                if(n % 16 < 10) {
                                    ...
                                    create_cube(pos, main_ori, axiom::sqrt3 / 3.0f, color);
                                } else if(n % 16 < 12) {
                                    vec3 color = axiom::hsv_color(rand() * 0.125f + 0.875f, 0.8f, 1.0f);

                                    create_tetrahedron(pos, main_ori, axiom::sqrt3 / 3.0f, color);
                                } else if(n % 16 < 14) {
                                    vec3 color = axiom::hsv_color(rand() * 0.125f + 3.875f, 0.8f, 1.0f);
                                    uint n = rand.next() % 5 + 3;

                                    create_bipyramid(pos, main_ori, axiom::sqrt3 / 3.0f, n, color);
                                } else if(n % 16 < 15) {
                                    vec3 color = axiom::hsv_color(rand() * 0.125f + 0.3f, 0.8f, 1.0f);

                                    create_dodecahedron(pos, main_ori, axiom::sqrt3 / 3.0f, color);
                                } else if(n % 16 < 16) {
                                    vec3 color = axiom::hsv_color(rand() * 0.125f + 3.0f, 0.8f, 1.0f);

                                    create_icosahedron(pos, main_ori, axiom::sqrt3 / 3.0f, color);
                                }
                            }
                        }
                    }
