...
std::unordered_map<uint, ttf_glyph> glyphs;
...
void process_ttf(std::string filepath) {
                ...
                for(int i = 0; i < seg_count; ++i) {
                    ...
                    for(int j = start; j <= end; ++j) {
                        ...
                        if(id_range_offset[i] == 0) {
                            ...
                            address += uint(id_range_offset[i]) + (j - start) * 2;
                            cursor = address;
...

    {
        ...
        if(loca_format == 0) {
            for(int i = 0; i < glyphs.size(); ++i) {
                ...
                glyphs[i].glyph_address = address;
                
                std::cout << "glyph data: " << " " << glyphs[i].glyph_id << " " << glyphs[i].codepoint << " " << glyphs[i].glyph_address << " " << std::endl;
            }
    ...
    for(auto& [k, glyph] : glyphs) {
        cursor = table.offset + glyph.glyph_address;

        // read header
        
        std::cout << "glyph data: " << " " << glyph.glyph_id << " " << glyph.codepoint << " " << glyph.glyph_address << " " << std::endl;
        ...
        if(num_contours > 0) {
            ...
            for(int i = 0; i < num_contours; ++i) {
                ...
                ttf_contour contour;
                contour.points = std::vector<vec2>(points.begin() + start_index, points.begin() + (end_index + 1));
...
void render_lines(uint camera, std::vector<vec3> points) {
    static axiom::vertices vertices;
    if(!vertices.initialized) vertices.init();
    ...
    for(vec3 vvv : points) {
        ...
        vertex.position = vvv;
        vertex.color = vec4(1.0f, 0.25f, 0.25f, 1.0f);
...
int main(int argc, char** argv) {
    ...
    auto target_callback = [&window, camera_entity](axiom::render_target& target) {
        ...
        static axiom::vertices vs;
        if(!vs.initialized) vs.init();

        std::vector<vec3> cvs;

        static uint index = 0;
        if(window.pressed_buttons.contains(axiom::input_code::KEY_LEFT_ARROW)) --index;
        if(window.pressed_buttons.contains(axiom::input_code::KEY_RIGHT_ARROW)) ++index;

        for(auto& contour : glyphs[index].contours) {
            for(int i = 0; i < contour.points.size(); ++i) {

                cvs.push_back(vec3(contour.points[i], 5.0f));
                cvs.push_back(vec3(contour.points[(i + 1) % contour.points.size()], 5.0f));
            }
        }

        render_lines(camera_entity, cvs);
    ...
    std::vector<axiom::texture_format> format = {axiom::texture_format::RGBA8, axiom::texture_format::RGBA8, axiom::texture_format::RGBA8, axiom::texture_format::DEPTHF};
    std::vector<axiom::texture_attachment> attachment = {axiom::texture_attachment::COLOR0, axiom::texture_attachment::COLOR1, axiom::texture_attachment::COLOR2, axiom::texture_attachment::DEPTH};
    axiom::render_target render_target = axiom::render_target::create(
        target_callback,
        ivec2(512),
        format,
        attachment,
    ...
    axiom::window_widget::insert("AXIOM", ivec2(200), ivec2(100), axiom::color_purple);
    axiom::panel_widget::insert();
    axiom::scroll_widget::insert(6.0f, true);
    axiom::column_widget::insert();
    axiom::text_widget::insert(
        "In porta justo vitae lacinia interdum. Ut fringilla blandit ante in scelerisque. In sagittis, purus eu vestibulum venenatis, lorem ex varius ipsum, ac gravida elit magna eu nulla. Nam efficitur ex sit amet ex lobortis tincidunt. Vestibulum interdum tellus a mauris tempor ultricies. Aliquam erat volutpat. In hac habitasse platea dictumst. Etiam molestie mattis dui. Nam eu lacus sit amet massa commodo mollis ac elementum diam. Ut laoreet purus aliquam elit suscipit pulvinar vel vitae arcu. Phasellus elementum accumsan neque, non vehicula massa euismod quis. Vestibulum quis malesuada ligula. Aliquam erat volutpat. Etiam at aliquet urna. Donec at magna dictum, finibus urna ac, facilisis eros. Integer tellus nunc, tincidunt ac scelerisque dictum, molestie in ipsum.",
        axiom::text_alignment::LEFT
    );
    ...
    process_ttf("res/Caveat-Regular.ttf");
